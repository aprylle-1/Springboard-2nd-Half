...
router.put('/:id', async function (req,res,next){
    try{
        ...
        if (!amt || paid == undefined){
            throw new ExpressError ("Missing parameters. Amount or Paid is required", 400)
        }
        if (paid === true){
            paid_date = new Date()
        }
            ...
            throw new ExpressError("Parameter paid must either be true or false", 400)
        }
        const results = await db.query(`UPDATE invoices SET amt=$1, paid_date = $2, paid = $3 WHERE id=$4 RETURNING *`, [amt, paid_date, paid, req.params.id])
